...
from core.models import Node, Parent, RedFlag, TreeValidationResult, Triaging
...
class SQLiteRepository:
    ...
    def __init__(self, db_path: str | None = None):
        ...
        logger.debug(
            f"SQLiteRepository: After init, DB size: {os.path.getsize(self._db_path)} bytes"
        )
    ...
    def get_parent(self, node_id: int) -> Node | None:
    ...
    def get_parent_with_children(self, parent_id: int) -> Parent | None:
    ...
    def get_next_incomplete_parent(self) -> dict[str, Any] | None:
        ...
        with self._get_connection() as conn:
            ...
            row = cursor.fetchone()
            if row:
                return {"parent_id": row["parent_id"], "missing_slots": row["missing_slots"]}
            return None

    # Tree operations

    def upsert_children_atomic(
        self, parent_id: int, children_data: list[dict[str, Any]]
    ) -> dict[str, Any]:
        """
        ...
        with self._get_connection() as conn:
            try:
                ...
                if parent.depth >= 5:
                    raise ValueError(
                        f"Parent node {parent_id} is a leaf node and cannot have children"
                    )
    ...
    def validate_tree(self) -> TreeValidationResult:
        ...
        return TreeValidationResult(
            is_valid=len(violations) == 0, violations=violations, summary=summary
        )
        ...
        with self._get_connection() as conn:
            try:
                ...
                cursor.execute(
                    ...
                    VALUES (?, ?, ?)
                """,
                    (node_id, red_flag_id, self._datetime_to_iso(datetime.now())),
                )
    ...
    def get_triage(self, node_id: int) -> Triaging | None:
    ...
    def update_triage(
        self, node_id: int, diagnostic_triage: str | None, actions: str | None
    ) -> bool:
        ...
        with self._get_connection() as conn:
            try:
                ...
                cursor.execute(
                    ...
                    VALUES (?, ?, ?, ?)
                """,
                    (node_id, diagnostic_triage, actions, self._datetime_to_iso(datetime.now())),
                )
    ...
    def get_parents_with_missing_slots(self) -> list[dict[str, Any]]:
        ...
        with self._get_connection() as conn:
            ...
            for row in cursor.fetchall():
                existing_slots = (
                    [int(s) for s in row["existing_slots"].split(",") if s]
                    if row["existing_slots"]
                    else []
                )
    ...
    def create_red_flag_audit(
        self, node_id: int, flag_id: int, action: str, user: str | None = None
    ) -> int:
        ...
        with self._get_connection() as conn:
            try:
                ...
                cursor.execute(
                    ...
                    VALUES (?, ?, ?, ?, ?)
                """,
                    (node_id, flag_id, action, user, self._datetime_to_iso(datetime.now())),
                )
    ...
    def get_red_flag_audit_by_id(self, audit_id: int) -> dict[str, Any] | None:
    ...
    def get_red_flag(self, flag_id: int) -> RedFlag | None:
    ...
    def get_duplicate_labels(self, limit: int = 100, offset: int = 0) -> list[dict[str, Any]]:
        ...
        with self._get_connection() as conn:
            ...
            for row in rows:
                ...
                child_ids = (
                    [int(id_str) for id_str in child_ids_str.split(",") if id_str]
                    if child_ids_str
                    else []
                )
                result.append(
    ...
    def get_orphan_nodes(self, limit: int = 100, offset: int = 0) -> list[dict[str, Any]]:
        ...
        with self._get_connection() as conn:
            ...
            for row in rows:
                ...
                result.append(
                    {"id": node_id, "parent_id": parent_id, "label": label, "depth": depth}
                )
            return result
    ...
    def get_import_job(self, job_id: int) -> dict[str, Any] | None:

...
def validate_phrase(field: str, s: str):
    ...
    if not s:
        raise HTTPException(
            status_code=422,
            detail=[
                {"loc": ["body", field], "msg": "must not be empty", "type": "value_error.empty"}
            ],
        )
    ...
    if not ALLOWED_RE.match(s):
        raise HTTPException(
            status_code=422,
            detail=[
                {"loc": ["body", field], "msg": "invalid characters", "type": "value_error.regex"}
            ],
        )
    ...
    if PROHIBITED_TOKENS.search(s):
        raise HTTPException(
            ...
            detail=[
                {
                    "loc": ["body", field],
                    "msg": "dosing/route tokens prohibited",
                    "type": "value_error.prohibited_token",
                }
            ],
        )
...
def clamp_llm_suggestion(text: str | None) -> str:
    """
    ...
    normalized = re.sub(
        rf"\b{PROHIBITED_TOKENS}\b", "[medication]", normalized, flags=re.IGNORECASE
    )

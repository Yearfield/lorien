...
from .models import TreeValidationResult
        ...
        for _, row in df.iterrows():
            ...
            if parent_val and child_val and parent_val != "" and child_val != "":
                ...
                parent_variants[parent_val].append(
                    {"parent_path": parent_path, "children": [child_val]}
                )
...
def enforce_five_children(
    parent_id: int, df: pd.DataFrame, level: int, parent_label: str, new_children: list[str]
) -> pd.DataFrame:
    """
...
def validate_tree_structure(df: pd.DataFrame) -> TreeValidationResult:
    ...
    if not validate_canonical_headers(df):
        ...
        return TreeValidationResult(
            is_valid=False, violations=violations, summary={"total_violations": len(violations)}
        )
...
def _build_parent_path(
    df: pd.DataFrame, level: int, parent_label: str, row: pd.Series | None = None
) -> str:

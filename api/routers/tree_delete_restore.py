...
def _subtree_snapshot(conn: sqlite3.Connection, node_id: int) -> Dict[str, Any]:
    ...
    """
    rows = [dict(id=r[0], parent_id=r[1], depth=r[2], slot=r[3], label=r[4]) for r in conn.execute(sql, (node_id,)).fetchall()]
    if not rows:
...
def delete_node(
  ...
  snap = _subtree_snapshot(conn, node_id)
  if dry_run:
    return {"dry_run": True, "snapshot": snap}
  # Apply delete transactionally
...
def restore_subtree(
  ...
  if parent_id is not None:
    row = conn.execute("SELECT depth FROM nodes WHERE id=?", (parent_id,)).fetchone()
    if not row:
  ...
  if parent_id is not None:
    cnt = conn.execute("SELECT COUNT(*) FROM nodes WHERE parent_id=?", (parent_id,)).fetchone()[0]
    # if the origin was a sibling under parent, we are adding 1
    if cnt >= 5:
      raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                          detail=[{"type":"value_error.max_children","msg":"parent already has 5 children"}])
  ...
  try:
    ...
    ins = conn.execute(
      ...
      (parent_id, (parent_depth + 1) if parent_depth is not None else 0, origin.get("slot"), origin.get("label")),
    )
    ...
    for n in nodes[1:]:
      ...
      child_depth = parent_depth + 1 + (n.get("depth", 0) - origin.get("depth", 0))
      ins2 = conn.execute(
        "INSERT INTO nodes (parent_id, depth, slot, label) VALUES (?,?,?,?)",

...
class ValidationError(DecisionTreeAPIException):
    ...
    def __init__(self, detail: str):
        super().__init__(
            status_code=status.HTTP_400_BAD_REQUEST, detail=detail, code="VALIDATION_ERROR"
        )
...
class ConflictError(DecisionTreeAPIException):
    ...
    def __init__(self, detail: str):
        super().__init__(status_code=status.HTTP_409_CONFLICT, detail=detail, code="CONFLICT_ERROR")
...
def handle_integrity_error(request: Request, exc: sqlite3.IntegrityError) -> JSONResponse:
    ...
    return JSONResponse(
        ...
        content={"error": "Database Integrity Error", "detail": error_detail, "code": error_code},
    )
...
def handle_decision_tree_api_exception(
    ...
    return JSONResponse(
        status_code=exc.status_code, content={"error": exc.detail, "code": exc.code}
    )

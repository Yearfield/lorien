    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            if response.status_code == 409:
                increment_counter("conflicts", tags={"path": self._normalize_path(path)})
            elif response.status_code == 422:
                increment_counter("validation_errors", tags={"path": self._normalize_path(path)})
            ...
            record_timer(
                "http_response_time",
                duration_ms,
                tags={"method": method, "path": self._normalize_path(path), "status_code": "500"},
            )

            increment_counter("http_server_errors")
            increment_counter(
                "http_exceptions",
                tags={"exception_type": type(e).__name__, "path": self._normalize_path(path)},
            )

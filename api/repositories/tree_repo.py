...
class TreeRepository:
    ...
    async def next_underfilled_parent(self, after_id: int | None = None) -> dict | None:
        ...
        LIMIT 1;
        """
        row = await anyio.to_thread.run_sync(self.conn.execute(q, (after_id, after_id)).fetchone)
        if not row:
            return None
        return {"id": row[0], "label": row[1], "depth": row[2], "child_count": row[3]}

    async def next_underfilled_parent_scoped(
        self, root_id: int, after_id: int | None
    ) -> dict | None:
        """
      ...
      WHERE parent_id IS NOT NULL
        ...
        row = await anyio.to_thread.run_sync(
            self.conn.execute(q1, (root_id, after_id, after_id)).fetchone
        )
        if not row:
            ...
            LIMIT 1
            """
            row = await anyio.to_thread.run_sync(self.conn.execute(q2, (root_id,)).fetchone)
        if not row:
            return None
        return {"id": row[0], "label": row[1], "depth": row[2], "child_count": row[3]}

    async def set_edge_flag(self, parent_id: int, child_id: int, red_flag: bool) -> None:
    ...
    async def list_children(self, parent_id: int, only_red: bool = False) -> list[dict]:
        ...
        """
        rows = await anyio.to_thread.run_sync(self.conn.execute(q, (parent_id, parent_id)).fetchall)
        items = [
            {"id": r[0], "label": r[1], "depth": r[2], "slot": r[3], "red_flag": int(r[4]) == 1}
            for r in rows
        ]
        if only_red:
    ...
    async def find_clone_candidates_by_label(self, label: str) -> list[dict]:
        ...
        """
        rows = await anyio.to_thread.run_sync(self.conn.execute(q, (label,)).fetchall)
        return [{"id": r[0], "label": r[1], "depth": r[2], "child_count": r[3]} for r in rows]

    async def clone_subtree(self, source_root_id: int, new_parent_id: int) -> int:
        """
        ...
        """
        root_row = await anyio.to_thread.run_sync(
            self.conn.execute(
        ...
        dest_row = await anyio.to_thread.run_sync(
            self.conn.execute(
    ...
    async def get_ancestors(self, node_id: int):
        """
        ...
        ORDER BY depth ASC;
        """
        rows = await anyio.to_thread.run_sync(self.conn.execute(q, (node_id,)).fetchall)

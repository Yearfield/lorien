...
class EnhancedTreeRepository:
    ...
    def __init__(self, use_cache: bool = True, use_monitoring: bool = True):
        ...
        logger.info(
            f"Enhanced repository initialized: cache={use_cache}, monitoring={use_monitoring}"
        )
    ...
    async def _execute_query(
        ...
        if self.use_cache and self._cache and cache_key:
            cached_result = await self._cache.get(query=cache_key, params=params, tags=cache_tags)
        ...
        try:
            async with self._get_connection() as conn:
                if fetch == "one":
                    result = await asyncio.to_thread(lambda: conn.execute(query, params).fetchone())
                elif fetch == "all":
                    result = await asyncio.to_thread(lambda: conn.execute(query, params).fetchall())
                else:
                    result = await asyncio.to_thread(lambda: conn.execute(query, params))
            ...
            if self._monitor:
                self._monitor.record_query_execution(
                    query=query, execution_time_ms=execution_time_ms, error=error
                )

        # Cache result
        if self.use_cache and self._cache and cache_key and result is not None:
            await self._cache.set(
                query=cache_key, value=result, params=params, ttl_seconds=cache_ttl, tags=cache_tags
            )
    ...
    async def update_triage(
        ...
        async with self._get_connection() as conn:
            cursor = await asyncio.to_thread(
                lambda: conn.execute(f"UPDATE triage SET {set_clause} WHERE node_id = ?", params)
            )
    ...
    async def invalidate_cache(
        self, tags: Optional[set[str]] = None, pattern: Optional[str] = None
    ):

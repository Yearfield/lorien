...
class SecurityConfig:
    ...
    def _load_config(self):
        ...
        # Security headers
        self.security_headers_enabled = self.is_production or bool(
            os.getenv("SECURITY_HEADERS_ENABLED", "false").lower() == "true"
        )
        ...
        self.security_logging_enabled = bool(
            os.getenv("SECURITY_LOGGING_ENABLED", "true").lower() == "true"
        )
        self.log_failed_auth_attempts = bool(
            os.getenv("LOG_FAILED_AUTH_ATTEMPTS", "true").lower() == "true"
        )
    ...
    def log_security_event(
        self, event_type: str, request: Request, details: Optional[dict[str, Any]] = None
    ):
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if self._is_rate_limited(client_ip, current_time):
            security_config.log_security_event(
                "rate_limit_exceeded",
                request,
                {"client_ip": client_ip, "requests_in_window": self._get_request_count(client_ip)},
            )
    ...
    def _record_request(self, client_ip: str, current_time: float):
        ...
        if client_ip not in self.request_counts:
            self.request_counts[client_ip] = {"first_request": current_time, "requests": []}
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if content_length and int(content_length) > security_config.max_request_size:
            security_config.log_security_event(
                "request_too_large",
                request,
                {"content_length": content_length, "max_allowed": security_config.max_request_size},
            )
        ...
        if self._contains_dangerous_patterns(str(request.url)):
            security_config.log_security_event(
                "suspicious_input", request, {"input_type": "url", "url": str(request.url)}
            )

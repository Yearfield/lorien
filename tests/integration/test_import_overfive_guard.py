...
def test_put_children_rejects_more_than_five(tmp_path):
    ...
    payload = {
        "parent_id": root_id,
        "children": [{"label": f"C{i}"} for i in range(1, 7)]
    }
    r = client.put("/api/v1/tree/children", json=payload)
    assert r.status_code == 422
    detail = r.json()["detail"]
    assert any(d.get("type") == "value_error.max_children" for d in (detail if isinstance(detail, list) else [detail]))
...
def test_import_rollback_if_overfive(tmp_path):
    ...
    r = client.post("/api/v1/import", params={"mode":"append","enforce_five":"true"}, files=files)

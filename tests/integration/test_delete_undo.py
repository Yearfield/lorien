...
def test_delete_and_undo(client: TestClient):
    ...
    r = client.post("/api/v1/tree/roots", json={"label":"Root A"}); assert r.status_code==201
    rid = r.json()["id"]
    put = client.put("/api/v1/tree/children", json={"parent_id": rid, "children":[{"label":"x"},{"label":"y"}]})
    assert put.status_code==200
    kids = client.get(f"/api/v1/tree/children?parent_id={rid}").json()["items"]
    kid_id = kids[0]["id"]
    # dry-run + delete
    dry = client.delete(f"/api/v1/tree/node/{kid_id}?dry_run=true"); assert dry.status_code==200
    snap = dry.json()["snapshot"]
    do = client.delete(f"/api/v1/tree/node/{kid_id}?dry_run=false"); assert do.status_code==200
    # verify gone
...
def test_restore_violates_five_children_limit(client: TestClient):
    ...
    r = client.post("/api/v1/tree/roots", json={"label":"Root A"}); assert r.status_code==201
    rid = r.json()["id"]
    put = client.put("/api/v1/tree/children", json={
        "parent_id": rid, 
        "children":[{"label":"x1"},{"label":"x2"},{"label":"x3"},{"label":"x4"},{"label":"x5"}]
    })
    assert put.status_code==200
    
    # Create another root with a child to delete
    r2 = client.post("/api/v1/tree/roots", json={"label":"Root B"}); assert r2.status_code==201
    rid2 = r2.json()["id"]
    put2 = client.put("/api/v1/tree/children", json={"parent_id": rid2, "children":[{"label":"y"}]})

...
def _get_child_id_by_label(client: TestClient, parent_id: int, label: str) -> int:
    ...
    raise AssertionError(
        f"Child with label '{label}' not found under parent {parent_id}. Got: {kids}"
    )
...
def test_data(temp_db):
    ...
    r = client.put(
        "/api/v1/tree/children", json={"parent_id": root1, "children": [{"label": "hypertension"}]}
    )
    assert r.status_code == 200, r.text
    ...
    root2 = r.json()["id"]
    r = client.put(
        "/api/v1/tree/children", json={"parent_id": root2, "children": [{"label": "hypertension"}]}
    )
    assert r.status_code == 200, r.text
    ...
    root3 = r.json()["id"]
    r = client.put(
        "/api/v1/tree/children", json={"parent_id": root3, "children": [{"label": "hypertension"}]}
    )
    assert r.status_code == 200, r.text
    ...
    for label in ["level1", "level2", "level3", "level4", "level5"]:
        r = client.put(
            "/api/v1/tree/children",
            json={"parent_id": current_parent, "children": [{"label": label}]},
        )
        assert r.status_code == 200, r.text
    ...
    r = client.put(
        "/api/v1/tree/children",
        json={"parent_id": current_parent, "children": [{"label": "hypertension"}]},
    )
    assert r.status_code == 200, r.text
...
def test_scan_conflicts(test_data):
    ...
    assert (
        len(grp["union_children"]) > 5
    )  # union: headache, chest pain, myalgia, dizziness, nausea, vomiting = 6
...
def test_resolve_conflict_all_parents_max_depth(temp_db):
    ...
    r = client.put(
        "/api/v1/tree/children",
        json={"parent_id": current_parent, "children": [{"label": max_depth_label}]},
    )
    assert r.status_code == 200, r.text

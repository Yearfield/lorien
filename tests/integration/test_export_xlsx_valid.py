...
def client(tmp_path, monkeypatch):
    ...
    c = TestClient(app)
    # Build minimal tree
    r = c.post("/api/v1/tree/roots", json={"label": "Root A"}); assert r.status_code == 201
    rid = r.json()["id"]
    assert c.put("/api/v1/tree/children", json={"parent_id": rid, "children": [{"label":"child1"},{"label":"child2"}]}).status_code == 200
    return c

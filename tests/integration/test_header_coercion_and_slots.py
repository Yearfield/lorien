...
def test_import_over_five_children_and_unique_slots(client: TestClient):
    ...
    r = client.post(
        "/api/v1/import?mode=replace", files={"file": ("over5.csv", CSV_OVER5, "text/csv")}
    )
    ...
    with _connect_env_db() as conn:
        ...
        cur.execute(
            "SELECT slot FROM nodes WHERE parent_id = ? AND depth = 2 ORDER BY slot", (parent_id,)
        )
...
def test_header_coercion_lowercase_notes_does_not_shift_d0(client: TestClient):
    ...
    r = client.post(
        ...
        files={"file": ("notes_lower.csv", CSV_NOTES_LOWERCASE, "text/csv")},
    )
    assert r.status_code == 200, r.text

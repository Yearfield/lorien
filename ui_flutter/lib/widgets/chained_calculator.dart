...
class ChainedCalculator extends StatefulWidget {
  const ChainedCalculator({super.key, required this.baseUrl, required this.dio});
  final String baseUrl;
  final Dio dio;
  @override State<ChainedCalculator> createState() => _ChainedCalculatorState();
}
...
class _ChainedCalculatorState extends State<ChainedCalculator> {
  ...
  @override void initState(){ super.initState(); _loadRoot(); }

  Future<void> _loadRoot() async {
    setState(()=>loading=true);
    final r = await widget.dio.get('${widget.baseUrl}/calc/options', queryParameters: {});
    setState((){ optsRoot = List<String>.from(r.data['roots']??[]); loading=false; });
  }
  ...
  Widget _dd(String? val, List<String> opts, void Function(String?) onChanged, String hint){
  ...
  @override Widget build(BuildContext context) {
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        _dd(root, optsRoot, (v){ setState(()=>root=v); _resetFrom(1); _loadNext(depth:1); }, 'Vital Measurement'),
        _dd(n1, opts1, (v){ setState(()=>n1=v); _resetFrom(2); _loadNext(depth:2); }, 'Node 1'),
        _dd(n2, opts2, (v){ setState(()=>n2=v); _resetFrom(3); _loadNext(depth:3); }, 'Node 2'),
        _dd(n3, opts3, (v){ setState(()=>n3=v); _resetFrom(4); _loadNext(depth:4); }, 'Node 3'),
        _dd(n4, opts4, (v){ setState(()=>n4=v); _resetFrom(5); _loadNext(depth:5); }, 'Node 4'),
        _dd(n5, opts5, (v){ setState(()=>n5=v); _loadNext(depth:6); }, 'Node 5'),

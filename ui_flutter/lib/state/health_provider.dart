...
class HealthStatus {
  const HealthStatus({required this.ok, required this.version, required this.lastPing});
  final bool ok;
...
class HealthController extends AsyncNotifier<HealthStatus?> {
  ...
  Future<HealthStatus?> ping() async {
    ...
    try {
      ...
      final st = HealthStatus(ok: ok, version: version, lastPing: DateTime.now());
      _last = st.lastPing;
      state = AsyncData(st);
      return st;
    } catch (e, st) {
...
final healthControllerProvider = AsyncNotifierProvider<HealthController, HealthStatus?>(() => HealthController());

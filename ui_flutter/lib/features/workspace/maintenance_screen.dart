...
class _MaintenanceScreenState extends State<MaintenanceScreen> {
  ...
  Future<void> _loadStats() async {
    try {
      final resp = await _http.get(Uri.parse('${widget.baseUrl}/api/v1/tree/stats'));
      if (resp.statusCode == 200) {
        ...
        setState(() { _error = 'Stats HTTP ${resp.statusCode}'; });
      }
    } catch (e) {
      setState(() { _error = 'Stats failed: $e'; });
    }
  }

  Future<void> _clear() async {
    setState(() { _loading = true; _error = null; _message = null; });
    final uri = Uri.parse('${widget.baseUrl}/api/v1/admin/clear?include_dictionary=${_includeDictionary.toString()}');
    try {
      final resp = await _http.post(uri);
      if (resp.statusCode == 200) {
        final body = jsonDecode(resp.body) as Map<String, dynamic>;
        setState(() { _message = 'Workspace cleared. Dictionary: ${body["dictionary_cleared"] == true ? "cleared" : "untouched"}'; _confirm = false; });
      } else {
        setState(() { _error = 'Clear HTTP ${resp.statusCode}'; });
      }
    } catch (e) {
      setState(() { _error = 'Clear failed: $e'; });
    } finally {
          ...
          children: [
            ...
            Row(
              children: [
                ...
                const Expanded(child: Text('I understand this will erase all current data in the workspace.')),
              ],
            ),
            Row(
              children: [

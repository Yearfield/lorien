...
class VMBuilderScreen extends StatefulWidget {
  ...
  const VMBuilderScreen({super.key, this.baseUrl = 'http://127.0.0.1:8000', this.client});
  @override
...
class _VMBuilderScreenState extends State<VMBuilderScreen> {
  ...
  final Map<int, TextEditingController> _slotCtl = { for (var i=1;i<=5;i++) i: TextEditingController() };
  final TextEditingController _vmCtl = TextEditingController();
  String? _err; bool _loading = false;

  @override
  void dispose(){ for (final c in _slotCtl.values) { c.dispose(); } _vmCtl.dispose(); super.dispose(); }

  Future<void> _createVM() async {
    final label = _vmCtl.text.trim();
    if (label.isEmpty) { setState(()=> _err='Enter a VM label'); return; }
  ...
  Future<void> _loadChildren(int parentId) async {
    final r = await _http.get(Uri.parse('${widget.baseUrl}/api/v1/tree/children?parent_id=$parentId'));
    if (r.statusCode==200){
      final m = jsonDecode(r.body);
      setState(()=> _children = List<Map<String,dynamic>>.from(m['items'].map((e)=> Map<String,dynamic>.from(e))));
      // prefill slot fields
      for (var i=1;i<=5;i++){
        final ex = _children.firstWhere((e)=> e['slot']==i, orElse: ()=> {});
        _slotCtl[i]!.text = ex.isNotEmpty ? (ex['label']?.toString() ?? '') : '';
      }
    }
  }
  ...
  Future<void> _loadSuggestionsForLabel(String label) async {
    ...
    if (r.statusCode==200){
      final m = jsonDecode(r.body);
      setState(()=> _suggestions = List<Map<String,dynamic>>.from(m['union']));
  ...
  Future<void> _saveSlot(int slot) async {
    ...
    if (r.statusCode==200){
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Save slot $slot failed: ${r.statusCode}')));
    }
  }
        ...
        children: [
          ...
          SizedBox(
            ...
            child: ListView(
              ...
              children: [
                const Text('Create Vital Measurement', style: TextStyle(fontWeight: FontWeight.bold)),
                const SizedBox(height: 8),
                TextField(controller: _vmCtl, decoration: const InputDecoration(labelText: 'VM label', border: OutlineInputBorder())),
                const SizedBox(height: 8),
                ElevatedButton.icon(onPressed: _createVM, icon: const Icon(Icons.add), label: const Text('Create / Use')),
                const Divider(),
                if (_root!=null) ...[
                  Text('Current VM: ${_root!['label']}'),
                  const SizedBox(height: 8),
                  const Text('Navigate children:', style: TextStyle(fontWeight: FontWeight.w600)),
                  ..._children.map((c)=> ListTile(
                    title: Text(c['label']?.toString() ?? '(empty)'),
                    subtitle: Text('slot ${c['slot']} • depth ${c['depth']}'),
          ...
          Expanded(
            child: Padding(
              ...
              child: _currentParent==null
                ? const Center(child: Text('Create or select a parent on the left'))
                : Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text('Editing children of: ${_currentParent!['label']} (id=${_currentParent!['id']})'),
                    const SizedBox(height: 8),
                        ...
                        SizedBox(
                          ...
                          child: Row(children: [
                            Expanded(child: TextField(
                              ...
                              decoration: InputDecoration(labelText: 'Slot $i label', border: const OutlineInputBorder()),
                              onSubmitted: (_)=> _saveSlot(i),
                            )),
                            const SizedBox(width: 8),
                            ElevatedButton(onPressed: ()=> _saveSlot(i), child: const Text('Save'))
                          ]),
                        ),
                    ]),
                    const Divider(),
                    ...
                    Wrap(spacing: 8, runSpacing: 8, children: _suggestions.map((s) =>
                      ActionChip(label: Text('${s["label"]} (${s["freq"]})'),
                        onPressed: (){
                          // put into the first empty slot
                          for (var i=1;i<=5;i++){
                            if (_slotCtl[i]!.text.trim().isEmpty){
                              _slotCtl[i]!.text = s["label"].toString();
                              _saveSlot(i);
                              break;
                            }
                          }

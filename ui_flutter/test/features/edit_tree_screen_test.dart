...
void main() {
  testWidgets('EditTreeScreen shows parent list and handles slot editing', (tester) async {
    int getCount = 0;
    int putCount = 0;
    ...
    final mock = MockClient((req) async {
      ...
      if (req.method == 'PUT' && req.url.path.contains('/slot/')) {
        ...
        return http.Response(jsonEncode({"action": "created", "node_id": 99, "parent_id": 42, "slot": 3, "label": capturedBody["label"]}), 200);
      }
    ...
    expect(find.text('Fill missing slots (leave blank to skip):'), findsOneWidget);

...
st.set_page_config(page_title="Outcomes - Lorien", page_icon="📋", layout="wide")
...
with col1:
    search_query = st.text_input(
        "Search by triage text or actions", placeholder="e.g., chest pain, urgent care"
    )
...
try:
    if search_query or vm_filter:
        ...
        for outcome in results:
            with st.expander(
                f"Node {outcome.get('node_id', 'Unknown')}: {outcome.get('vital_measurement', 'No VM')}"
            ):
                ...
                # Edit button
                if st.button(
                    f"✏️ Edit Node {outcome.get('node_id', 'Unknown')}",
                    key=f"edit_{outcome.get('node_id', 'Unknown')}",
                ):
        ...
        if st.button("💾 Save Changes", type="primary"):
            if triage_text.strip() or actions_text.strip():
                try:
                    ...
                    result = put_json(
                        f"/triage/{node_id}",
                        {"triage": triage_text.strip(), "actions": actions_text.strip()},
                    )

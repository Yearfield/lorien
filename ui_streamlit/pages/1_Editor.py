...
st.set_page_config(page_title="Editor - Lorien", page_icon="✏️", layout="wide")
...
if st.session_state["parents_list"]:
    for parent in st.session_state["parents_list"]:
        ...
        with st.expander(
            f"Parent {parent_id}: {parent.get('label', 'No label')} - Missing: {missing_slots}"
        ):
            ...
            with col2:
                if st.button("📊 View Details", key=f"view_{parent_id}"):
                    st.session_state["detail_parent_id"] = str(parent_id)
                    st.switch_page("pages/2_Parent_Detail.py")
else:
...
with st.expander("New Vital Measurement Form"):
    new_vm_label = st.text_input(
        "Vital Measurement Label", placeholder="e.g., Chest Pain Assessment"
    )
    children_labels = []

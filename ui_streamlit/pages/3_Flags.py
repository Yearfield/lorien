...
from ui_streamlit.api_client import delete_json, get_json, post_json

st.set_page_config(page_title="Flags - Lorien", page_icon="🚩", layout="wide")
...
search_query = st.text_input(
    "Search flags by name or description", placeholder="e.g., urgent, critical, emergency"
)
    ...
    for flag in results:
        with st.expander(f"🚩 {flag.get('name', 'No name')}"):
            ...
            node_id = st.text_input(
                f"Node ID to assign {flag.get('name', 'flag')} to", key=f"node_{flag['id']}"
            )

            col1, col2 = st.columns(2)
            with col1:
                if st.button("✅ Assign to Node", key=f"assign_{flag['id']}"):
                    if node_id:
                        try:
                            result = post_json(
                                f"/flags/{flag['id']}/assign", {"node_id": int(node_id)}
                            )
                            st.success(
                                f"Flag {flag.get('name', 'flag')} assigned to node {node_id}"
                            )
                            st.rerun()
            ...
            with col2:
                if st.button("❌ Remove from Node", key=f"remove_{flag['id']}"):
                    if node_id:
                        try:
                            result = delete_json(f"/flags/{flag['id']}/assign/{node_id}")
                            st.success(
                                f"Flag {flag.get('name', 'flag')} removed from node {node_id}"
                            )
                            st.rerun()
                        except Exception as e:
...
try:
    ...
    if audit_entries:
        ...
        for entry in audit_entries[:10]:  # Show first 10
            with st.expander(
                f"{entry.get('action', 'Unknown')} - {entry.get('flag_name', 'Unknown flag')}"
            ):

...
from ui_streamlit.api_client import (
    get_json,
    put_json,
)

st.set_page_config(page_title="Parent Detail - Lorien", page_icon="✏️", layout="wide")
...
if parent_id:
    ...
    try:
        ...
        if bulk_edit:
            ...
            new_slot_labels = []
            for i in range(5):
                label = st.text_input(f"Slot {i+1}", value=slot_labels[i], key=f"bulk_slot_{i}")
                    ...
                    for i in range(5):
                        if new_slot_labels[i].strip():
                            children_data.append(
                                {"slot": i + 1, "label": new_slot_labels[i].strip()}
                            )

                    if children_data:
                        result = put_json(
                            f"/tree/{parent_id}/children", {"children": children_data}
                        )
            ...
            if children:
                for i, child in enumerate(children):
                    with st.expander(
                        f"Slot {child.get('slot', i+1)}: {child.get('label', 'No label')}"
                    ):
            ...
            if new_child_label:
                try:
                    ...
                    if next_slot <= 5:  # Enforce 5-child limit
                        new_child = put_json(
                            f"/tree/{parent_id}/children",
                            {"label": new_child_label, "slot": next_slot},
                        )

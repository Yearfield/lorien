...
st.set_page_config(page_title="Calculator - Lorien", page_icon="🧮", layout="wide")
...
try:
    ...
    if not roots:
        st.info(
            "🌱 No decision trees found — Import a workbook in **Workspace** or create a new **Vital Measurement**."
        )
        st.markdown(
            """
        **To get started:**
        ...
        try:
            ...
            if children:
                ...
                for child in children:
                    with st.expander(
                        f"Slot {child.get('slot', 'Unknown')}: {child.get('label', 'No label')}"
                    ):

...
def cmd_backup(args):
    ...
    target = (
        Path(args.target)
        if args.target
        else (db.parent / f"lorien_backup_{time.strftime('%Y%m%d_%H%M%S')}.db")
    )
...
def main():
    ...
    s = sub.add_parser("show-db-path")
    s.set_defaults(func=cmd_show_db_path)
    ...
    s = sub.add_parser("wal-checkpoint")
    s.set_defaults(func=cmd_wal_checkpoint)

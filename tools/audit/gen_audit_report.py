...
from _util import ROOT as REPO_ROOT
...
def main():
    ...
    pytest_summary = "\n".join(
        [l for l in out.splitlines() if l.strip().endswith("passed") or "failed" in l]
    )

    # Build sections
    route_rows = [[r["path"], ",".join(r["methods"]), r["name"]] for r in routes]
    dual_rows = [
        [base, "OK" if v["bare"] and v["v1"] else "MISSING", ", ".join(v["examples"])]
        for base, v in sorted(pairs.items())
    ]
    ...
    if not dual["ok"]:
        ...
        for base, v in dual["missing_dual_mounts"].items():
            md.append(
                f"- `{base}` → present: bare={v['bare']}, v1={v['v1']} · {', '.join(v['examples'])}"
            )

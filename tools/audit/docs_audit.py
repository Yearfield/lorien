...
def find_header_occurrences(content: str) -> list[str]:
    ...
    exact_matches = re.findall(
        r"Vital Measurement, Node 1, Node 2, Node 3, Node 4, Node 5, Diagnostic Triage, Actions",
        content,
    )
    ...
    for file_pattern in HEADER_FILES:
        ...
        for file_path in files_to_check:
            ...
            try:
                ...
                results["inconsistencies"].append({"file": file_path, "error": str(e)})
...
def check_required_docs() -> dict[str, Any]:
    ...
    results = {"required_docs": REQUIRED_DOCS, "missing_docs": [], "existing_docs": []}
...
def check_stale_docs() -> dict[str, Any]:
    ...
    results = {"stale_files": [], "threshold_days": STALE_THRESHOLD_DAYS}
                ...
                try:
                    ...
                    if age_seconds > threshold_seconds:
                        results["stale_files"].append(
                            {"file": file_path, "age_days": round(age_seconds / (24 * 60 * 60), 1)}
                        )
                except Exception as e:
                    results["stale_files"].append({"file": file_path, "error": str(e)})
...
def check_docs_vs_code() -> dict[str, Any]:
    ...
    results = {"endpoints_documented": [], "endpoints_missing_docs": [], "outdated_docs": []}
...
def main():
    ...
    # Overall status
    has_issues = (
        len(header_results["inconsistencies"]) > 0
        or len(docs_results["missing_docs"]) > 0
        or len(stale_results["stale_files"]) > 0
    )

...
def check_header_sot() -> dict[str, Any]:
    ...
    try:
        ...
        canonical_matches = [
            m
            for m in matches
            if "docs/API.md" in m and "Header (exact order; case-sensitive; comma-separated):" in m
        ]
        ...
        if canonical_count == 0:
            api_header_matches = [
                ...
                for m in matches
                if "docs/API.md" in m
                and "Vital Measurement, Node 1, Node 2, Node 3, Node 4, Node 5, Diagnostic Triage, Actions"
                in m
            ]
...
def validate_against_requirements(
    audit_results: dict[str, Any], requirements: dict[str, Any], header_check: dict[str, Any] = None
) -> dict[str, Any]:
    ...
    if coverage.get("missing_dual"):
        ...
        if other_missing:
            ...
            validation_results["errors"].append(
                f"Missing dual-mount for {len(other_missing)} routes: {[m['path'] for m in other_missing]}"
            )
        else:
            validation_results["dual_mount_compliant"] = True
            if health_missing:
                validation_results["warnings"].append(
                    f"Health endpoints missing bare mount (acceptable): {[m['path'] for m in health_missing]}"
                )
            if missing_dual and not non_conditional_missing:
                validation_results["warnings"].append(
                    f"Conditional endpoints missing dual-mount: {[m['path'] for m in missing_dual]}"
                )
    ...
    for cond_ep in conditional_endpoints:
        ...
        if path not in route_paths:
            validation_results["warnings"].append(
                f"Conditional endpoint not found: {path} ({cond_ep.get('description', '')})"
            )
    ...
    if not header_check.get("is_canonical", False):
        ...
        validation_results["errors"].append(
            f"Header SoT violation: found {header_check.get('match_count', 0)} instances, expected 1"
        )
...
def main():
    ...
    validation = validate_against_requirements(
        audit_results, requirements, audit_results.get("header_sot_check")
    )
    ...
    audit_results["summary"] = {
        ...
        "checks_run": ["routes_audit", "header_sot_check", "route_count_check", "validation"],
    }
